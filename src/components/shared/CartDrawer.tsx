          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-ocean-100 dark:border-ocean-800 shrink-0">
              <div className="flex items-center gap-2">
                ...
                {totalItems > 0 && (
                  <span className="ml-1 text-xs bg-ocean-700 text-white rounded-full px-2 py-0.5">
                    {totalItems}
                  </span>
                )}
              </div>
              <button
                onClick={close}
                className="p-1.5 rounded-lg hover:bg-ocean-100 dark:hover:bg-ocean-800 transition-colors"
            ...
            <div className="flex-1 overflow-y-auto py-4 px-5">
              {items.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center gap-4 text-center py-16">
                  <ShoppingBag size={48} className="text-ocean-200 dark:text-ocean-700" />
                  <p className="text-ocean-500 font-medium">Your cart is empty</p>
                  <p className="text-sm text-ocean-400">Add some premium seafood to get started!</p>
                  <Button variant="outline" size="sm" onClick={() => goTo(ROUTES.PRODUCTS)}>
                    Browse Products
                  </Button>
                </div>
              ) : (
                <div className="flex flex-col gap-4">
                  {items.map((item) => (
                    <div
                      key={item.productId}
                      className="flex gap-3 pb-4 border-b border-ocean-50 dark:border-ocean-800 last:border-0"
                    >
                      {/* Image */}
                      <div className="w-16 h-16 rounded-xl overflow-hidden shrink-0 bg-ocean-50 dark:bg-ocean-800">
                        {item.image ? (
                          <img
                            src={item.image}
                            alt={item.name}
                          ...
                          <div className="w-full h-full flex items-center justify-center">
                            <ShoppingBag size={20} className="text-ocean-200" />
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-ocean-900 dark:text-white leading-snug line-clamp-2">
                          {item.name}
                        </p>
            ...
            {items.length > 0 && (
              <div className="shrink-0 border-t border-ocean-100 dark:border-ocean-800 px-5 py-4 bg-ocean-50/50 dark:bg-ocean-900/50">
                ...
                {totalPrice < CONFIG.DELIVERY.FREE_DELIVERY_ABOVE && (
                  <div className="mb-4">
                    <p className="text-xs text-ocean-500 mb-1.5">
                      Add <span className="font-semibold text-ocean-700 dark:text-ocean-200">
                        {formatCurrency(CONFIG.DELIVERY.FREE_DELIVERY_ABOVE - totalPrice)}
                      </span> more for free delivery
                    </p>
                    <div className="w-full h-1.5 bg-ocean-200 dark:bg-ocean-700 rounded-full overflow-hidden">
                      <div
                ...
                <div className="space-y-1.5 mb-4">
                  ...
                  <div className="flex justify-between font-bold text-ocean-900 dark:text-white border-t border-ocean-100 dark:border-ocean-800 pt-2 mt-2">
                    <span>Total</span>
